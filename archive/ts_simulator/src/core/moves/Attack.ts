...
export default class Attack extends Move {
    ...
    execute(state: GameState) {
        ...
        if (!defender) {
            throw new Error("Wtf are you attacking?");
        }
        
        const enemy = state.tribes[defender.owner];
        ...
        if(isSkilledIn(attacker, SkillType.Convert)) {
            const index = enemy.units.findIndex(x => x.coords.idx == defender.coords.idx);
            
            xorUnit.owner(state, defender, enemy.id, pov.id);
            defender.owner = attacker.owner;
            enemy.units.splice(index, 1);
            
            undoChain.push(() => {
                xorUnit.owner(state, defender, pov.id, enemy.id);
                enemy.units.splice(index, 0, defender);
                defender.owner = enemy.id;
            });
        }
        // normal attack, and attacker is still alive
            ...
            if(defender.health <= 0 && isSkilledIn(attacker, SkillType.Persist)) {
            ...
            if(attacker.health > 0 && isSkilledIn(attacker, SkillType.Escape)) {
    ...
    riot(state: GameState): Branch {
        const pov = getPovTribe(state);
        const infiltrator = getUnitAt(state, this.getSrc())!;
        const cityIndex = this.getTarget();
        const cityTile = state.tiles[cityIndex];
        const enemyTribe = state.tribes[cityTile.owner];
        const cityTarget = enemyTribe.cities.find(x => x.tileIndex == cityIndex)!;
        ...
        cityTarget._territory.forEach(x => {
            if(state.tiles[x]._unitOwnerID || x == cityIndex) {
                return;
            }
            switch (state.tiles[x].type) {
        ...
        for (let _ = 0; _ < income; _++) {
            ...
            const result = summonUnit(state, unitType, tileIndex, false, true);
            if(result) {
