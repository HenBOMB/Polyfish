...
*Jul 22, 2026 — pre-registered before running. This is the search-side fix the whole EXP_ELO_004→018 label/reward campaign was papering over. The diagnostic chain (notes.md "Inner mechanics"; failure_mode.md FM-3; the §7 credit-assignment writeup) converged on: the flywheel can't turn on tempo because MCTS(π) ≈ π at the expansion fork — the search is single-player (opponent auto-skipped in-tree, game.rs), so it can never see "if I dawdle, Greedy takes the village," and the leaf value head is indifferent between the fork's near-identical immediate successors. No reward/label can fix a search that's structurally blind to the contest.*
