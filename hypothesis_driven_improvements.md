...
*From EXP_ELO_S0 on, entries incorporate an external literature review (`expert_review_plateau.md`, Jul 18 2026) that independently converged on the same top lever (a persistent KL-anchor to a reference policy, citing AlphaStar) and supplied the primacy-bias/plasticity-loss citations this campaign's own research pass had flagged as a gap. Its implementation-level claims were code-audited before acting on them; corrections are noted inline below.*
...
**Refuted as the tower's cause:** value-target miscalibration (the value head *wants* expansion, §3), production collapse (it produces, §4), expansion-targeting/won't-seek (it pursues 91–98%, §4), and KL-anchoring alone (EXP_ELO_013). **Live, sharpened suspects:** (a) **completion-under-contest** — the model tries but only ~half-completes, and self-play hides the contest; matches EXP_ELO_001's army 13 vs 30 and the "loses the fight for the central 3rd village" intuition; (b) **opportunity rate** — only ~0.9 clear openings/game arise (exploration/tempo). **The campaign's earlier "tower = refuses to expand / over-teches" framing, built on aggregate gauge curves + single-ply reads, is downgraded** by the turn-level data to "tries, half-completes, under-contests." **Next measurement (not yet run):** the identical turn-level pursuit-and-completion analysis with **Greedy as the opponent** (not self-play), plus logging what kills the ~50% of pursuits that stall — this is the read that would actually locate the remaining deficit, and per the session's own lesson it should be trusted over any further single-ply probe.

**Data / repro:** `turnstates_{towered,clone}/` (per-turn JSONL, 80 games each), `decision_traces_towered/` (99 tower-window forks); instrumentation = `--dump-turn-states` + `ThirdCity` trace trigger, both uncommitted in `src/bin/self_play.rs`; analysis scripts in the session scratchpad. Models: towered `gauge_1784449090_iter20` (`1573835f`), BC clone `run_1784017302_iter1_start` (`a3996362`).
