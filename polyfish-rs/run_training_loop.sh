        ...
        if [ -n "$GAUGE_W" ] && [ -n "$GAUGE_L" ]; then
            ...
            if [ "$GAUGE_ACTION" = "freeze" ]; then
                ...
                if [ -n "${IGNORE_PLATEAU:-}" ]; then
                    ...
                    echo "8-reading/40-iteration windows with no gain vs the"
                    echo "active anchor) -- IGNORE_PLATEAU=1 set, continuing"
                    echo "anyway. Since EXP_ELO_127 this gauge always runs"
                    echo "backend1=macro-mcts vs backend2=greedy (ANCHOR_PATH"
                    echo "is hardcoded empty), so it DOES measure the same"
                    echo "macro policy/value heads MACRO_GEN trains -- this is"
                    echo "not an orthogonal-proxy false positive, it's a real"
                    echo "no-further-gain-vs-greedy signal being overridden"
                    echo "deliberately. ladder.json still records the reading"
                    echo "normally either way."
                    echo "=================================================="
                else
                    echo "=================================================="
                    echo "PLATEAU STOP at iteration $i: two consecutive"
                    echo "8-reading/40-iteration windows with no gain vs the"
                    echo "active anchor (see ladder.json)."
