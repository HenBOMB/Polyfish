...
use crate::actions::{gain_stars, spend_stars, UndoCallback};
...
pub fn consume_resource(state: &mut GameState, tile_idx: i32, replace_type: Option<ResourceType>) -> UndoCallback {
    let old_resource = match state.resources.get(&tile_idx) {
        ...
        None => {
            ...
            if let Some(rt) = replace_type {
                 // Insert new
                 state.resources.insert(tile_idx, Some(crate::states::ResourceState {
                     resource_type: rt,
                     tile_index: tile_idx,
                 }));
                 // Undo removes
                 return Box::new(move |s| { s.resources.remove(&tile_idx); });
            }
...
pub fn harvest_resource(state: &mut GameState, tile_idx: i32) -> UndoCallback {
    // Get resource info
