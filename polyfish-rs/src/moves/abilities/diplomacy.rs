...
use crate::actions::{chain_undos, structure::create_structure, structure::destroy_structure, spend_stars, UndoCallback};
...
impl Move for EstablishEmbassyMove {
    ...
    fn execute(&self, state: &mut GameState) -> Result<MoveResult, String> {
        ...
        undos.push(create_structure(state, self.target_index, StructureType::Embassy, 1));
    ...
    fn describe(&self, _state: &GameState) -> String {
        format!("Establish embassy in tribe {} at {}", self.opponent_id, self.target_index)
    }
...
impl Move for DestroyEmbassyMove {
    ...
    fn describe(&self, _state: &GameState) -> String {
        format!("Destroy embassy with tribe {} at {}", self.opponent_id, self.target_index)
    }
...
pub fn generate_diplomacy_moves(state: &GameState, moves: &mut Vec<Box<dyn Move>>) {
    ...
    let has_diplomacy = crate::settings::technology::has_technology(&tribe.tech_vanilla, crate::types::TechnologyType::Diplomacy);

    for (&other_id, relation) in &tribe.relations {
        if other_id == pov_id { continue; }
        ...
        if relation.state == 1 {
            ...
            if relation.embassy_level == 0 && tribe.stars >= 5 {
                if let Some(other_capital) = crate::functions::get_capital_city(state, other_id) {
                    moves.push(Box::new(EstablishEmbassyMove::new(other_capital.idx, other_id)));
                }
            }

            // Only offer if NOT already at peace and NOT already offered
            // We need to check if WE have offered peace to THEM
            let i_offered = state.tribes.get(&other_id).map(|t| {
                t.relations.get(&pov_id).map(|r| r.has_offered_peace).unwrap_or(false)
            }).unwrap_or(false);
        ...
        if relation.embassy_level > 0 {
             if let Some(other_capital) = crate::functions::get_capital_city(state, other_id) {
                 moves.push(Box::new(DestroyEmbassyMove::new(other_capital.idx, other_id)));
             }
        }
    }
}
