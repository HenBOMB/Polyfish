...
/// Check if a technology is present in the tribe's tech list, even if not yet
/// "discovered" (used for unlocking units/actions during MCTS simulations).
...
/// `get_tech_unlocking_structure`/`get_tech_unlocking_unit`.
...
/// picking a specific reverse-lookup helper by hand.
