...
pub fn evaluate_gamestate(state: &GameState, player_id: PlayerId) -> f32 {
    let my_score = player::evaluate_player(state, player_id);
    ...
    for &opponent_id in state.tribes.keys() {
        if opponent_id != player_id {
            let opp_score = player::evaluate_player(state, opponent_id);
