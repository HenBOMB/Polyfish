...
pub enum MacroLeaf {
    ...
    /// edge's directive (which training painted) instead of the scripted goal
    /// (which it did not). Measured: the two disagree on 52% of turns and the
    /// repaint moves the value 1.46x the q-spread the tree resolves.
    NetAsymPaint,
...
pub fn enumerate_candidates_with_belief(
    ...
    if let Some(lane) = pick_save_lane(state, pov, counters.tier3_bought) {
        push(
            MacroGoal {
        ...
        for (i, cand) in candidates.iter().enumerate() {
            ...
            for h in 0..self.params.horizon.max(1) {
                let goal_h = if h == 0 {
                    ...
                    compute_macro_goal(&sim.state, pov, counters.tier3_bought)
                };
...
mod tests {
    ...
    fn candidates_sorted_deduped_base_first() {
        for seed in 0..4i64 {
            let game = generated_game(seed);
            let pov = game.state.settings.current_player_turn_id;
            let base = compute_macro_goal(&game.state, pov, 0);
            let cands =
