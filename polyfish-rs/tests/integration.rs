...
fn test_load_game_state_from_json() {
    ...
    match result {
        Ok(game) => {
            ...
            assert!(!game.state.tribes.is_empty(), "Should have at least one tribe");
            
            // Count units
            let pov_id = game.current_player_id();
            if let Some(tribe) = game.state.tribes.get(&pov_id) {
                println!("  Current player {} has {} units", pov_id, tribe.units.len());
                for unit in &tribe.units {
                    println!("    - Unit {:?} at {} (moved={}, attacked={})", 
            ...
            for m in &moves {
                match m.move_type() {
                    ...
                    polyfish::MoveType::Recover => recover_count += 1,
                    polyfish::MoveType::Disband => disband_count += 1,
            ...
            println!("    Recover moves: {}", recover_count);
            println!("    Disband moves: {}", disband_count);
            ...
            assert!(!moves.is_empty(), "Should have at least one legal move (end turn)");
        }
        Err(e) => {
