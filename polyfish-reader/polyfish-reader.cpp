...
int polyai(uintptr_t modBase, pid_t pid, bool prod) {
    ...
    uintptr_t gameManagerRoot = getPlace(pid, modBase + 0x36841a8, { 0xB8, 0x0 });
    ...
    // Write newline delimiter for daemon mode readline
    write(1, "\n", 1);
...
int main(int argc, char** argv) {
    ...
    // Daemon mode: loop reading commands from stdin
    std::cerr << "[daemon] ready" << std::endl;
    std::string line;
    while (std::getline(std::cin, line)) {
        if (line == "quit" || line == "exit") break;
        polyai(modBase, pid, prod);
        // Flush stdout so Python can read immediately
        fflush(stdout);
    }
    std::cerr << "[daemon] exiting" << std::endl;
    return 0;
}