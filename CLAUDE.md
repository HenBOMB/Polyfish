...
If you change layer shapes, channel counts, or head sizes in one, you must mirror it in the other (and in `features.rs` / `mapper.rs` constants). Current values: spatial channels 162 (incl. observation-memory/ghost + per-tile pursuit channel, Jul 2026), player-state dim 10, map 11×11, policy heads = action(11) + source + target + option(192), normalization = GroupNorm(GN_GROUPS=8) on the 64-filter trunk (no BatchNorm anywhere; the pool convs are fully linear — no norm and no activation, since an unnormed ReLU there dies irreversibly, Jul 2026; the value pool `v_pool_conv` is 8-channel since Jul 2026 to remove the bottleneck, policy pools stay 1-channel). Mismatches surface as safetensors load errors or silent garbage. Old narrower training data (154 pre-obs-memory, 161 pre-pursuit) is zero-padded at load by train.py (channels are only ever appended at the end of the layout); BatchNorm-era checkpoints are rejected at load by all backends (quarantined in `checkpoints/bn_era/`, usable only with pre-GN binaries).
