...
- `score.rs` — the single pricing table for score (city, park, tile, structure, unit) read by **both** the incremental `tribe.score` updates and the canonical recompute (`functions::calculate_detailed_tribe_score`). Score is the reward/value currency, so the two must never disagree: territory and structures are scored by *tile ownership* (moved by `claim_territory`), cities carry only their level, population and parks. `tests/score_parity.rs` walks random and conquest-biased playouts and fails on any divergence; `self_play` emits `score_drift_max` / `score_drift_games` per run (#40).
- `mapgen.rs`, `coords.rs`, `fow.rs`, `memory.rs`, `hash.rs` — map generation, coordinate/index math, fog-of-war, observation memory, scoring, state hashing. **Training runs with FOW enabled** (deliberate, to avoid learning cheating/FOW-less strategies).
...
If you change layer shapes, channel counts, or head sizes in one, you must mirror it in the other (**and** in `tch_network.rs` / `metal_network.rs`, and in `features.rs` / `mapper.rs` constants). Current values: spatial channels **142** (`features.rs` `NUM_CHANNELS`, `train.py` `SPATIAL_CHANNELS`; = 136 + 6 fog-memory channels), player-state dim **16** (`features.rs:216`, `train.py` `PLAYER_STATE_DIM`), map 11×11, 6 ResBlocks on a 64-filter trunk, policy heads = action + source + target + option(192), normalization = GroupNorm(`GN_GROUPS = 8`) — no BatchNorm in the current architecture; the 1-channel pool convs are fully linear (no norm, no activation, since an unnormed ReLU there dies irreversibly). Mismatches surface as safetensors load errors or silent garbage. Legacy 136-channel training data is zero-padded at load by `train.py` (`pad_spatial`, channels were appended at the end of the layout), as are pre-widening 11-column `action_type` targets. July-2026 BatchNorm-era checkpoints load **inference-only**: `network.rs` folds eval-mode BatchNorm into a per-channel affine (`Norm::BatchEval`, plus the BN+ReLU the July pooled policy conv had) and `tch_network.rs` mirrors it, while the training paths and `migrate_model.py:25-30` still reject them (`checkpoints/bn_era/`).
