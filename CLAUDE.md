...
If you change layer shapes, channel counts, or head sizes in one, you must mirror it in the other (and in `features.rs` / `mapper.rs` constants). Current values: spatial channels 161 (incl. observation-memory/ghost channels, Jul 2026), player-state dim 10, map 11×11, policy heads = action(11) + source + target + option(192), normalization = GroupNorm(GN_GROUPS=8) on the 64-filter trunk (no BatchNorm anywhere; the 1-channel pool convs have no norm). Mismatches surface as safetensors load errors or silent garbage. Old 154-channel training data is zero-padded at load by train.py (channels were appended at the end of the layout); BatchNorm-era checkpoints are rejected at load by all backends (quarantined in `checkpoints/bn_era/`, usable only with pre-GN binaries).
