...
function App() {
  ...
  const [metricsExtra, setMetricsExtra] = useState<any[]>([]);
  ...
  // Per-role tempo curves sidecar (run_id -> iter -> role -> turn -> fields),
  // plus CSV columns missing from the running server's /metrics schema.
  // Both change once per training iteration, so a slow poll is plenty.
  useEffect(() => {
    const fetchTempo = () => {
      fetch('/api/tempo-by-turn')
        .then(res => (res.ok ? res.json() : {}))
        .then(setTempo)
        .catch(() => { });
      fetch('/api/metrics-extra')
        .then(res => (res.ok ? res.json() : []))
        .then(setMetricsExtra)
        .catch(() => { });
    };
    fetchTempo();
    const id = setInterval(fetchTempo, 15000);
  ...
  // Flatten the tempo sidecar into per-iteration rows for MetricChart:
  // "net" = the training net's seats (model + model_vs_anchor, n-weighted),
  // "gre" = the Greedy anchor seat — probed at fixed turns.
  const tempoRows = useMemo(() => {
    if (runMetrics.length === 0) return [];
    const lastRun = runMetrics[runMetrics.length - 1].run_id;
    const runTempo = lastRun !== undefined ? tempo?.[String(lastRun)] : undefined;
    if (!runTempo) return [];
    const probe = (roles: any, roleNames: string[], turn: number, field: string) => {
      let sum = 0, n = 0;
      for (const r of roleNames) {
        const turns = roles[r];
        if (!turns) continue;
        const keys = Object.keys(turns).map(Number).filter(Number.isFinite); // skip "_totals"
        if (keys.length === 0) continue;
        let best = keys[0];
        for (const k of keys) if (Math.abs(k - turn) < Math.abs(best - turn)) best = k;
        if (Math.abs(best - turn) > 3) continue; // games ended well before this probe
        const d = turns[String(best)];
        const w = d.n ?? 1;
        sum += d[field] * w;
        n += w;
      }
    ...
    // Per-player-game totals from the role's "_totals" block (unbiased vs
    // reading the last turn key, which undercounts games that ended early).
    const probeTotals = (roles: any, roleNames: string[], field: string) => {
      let sum = 0, n = 0;
      for (const r of roleNames) {
        const tot = roles[r]?.['_totals'];
        if (!tot || typeof tot[field] !== 'number') continue;
        const w = tot.n_games ?? 1;
        sum += tot[field] * w;
        n += w;
      }
      return n > 0 ? sum / n : undefined;
  ...
  // CSV rows + tempo-derived fields joined by iteration, so one chart can
  // mix both sources (e.g. net counters from CSV, greedy's from the sidecar).
  // metrics-extra fills columns the running server's /metrics doesn't emit;
  // spread-order keeps a real /metrics value when both provide one.
  const combinedRows = useMemo(() => {
    if (runMetrics.length === 0) return tempoRows;
    const lastRun = runMetrics[runMetrics.length - 1].run_id;
    const byIter = new Map(tempoRows.map((r: any) => [r.iteration, r]));
    const extraByIter = new Map(
      metricsExtra
    ...
    return runMetrics.map(m => {
      ...
      // (income/city was healthy; city count was the gap), so it's worth
      // a first-class trend rather than re-deriving ad hoc each time.
      row.net_spt_per_city_t10 = divide(row.net_spt_t10, row.net_cities_t10);
        ...
        <Routes>
          ...
          <Route path="/metrics" element={
            <section>
              ...
              <div className="metrics-grid">
                {/* Tempo: cities curve, net seat vs the Greedy anchor */}
                <MetricChart
                  title="Cities by turn"
                  data={windowSize > 0 ? combinedRows.slice(-windowSize) : combinedRows}
                  series={[
                    { key: 'net_cities_t10', label: 'net @t10', color: '#22c55e' },
                    { key: 'net_cities_t20', label: 'net @t20', color: '#15803d' },
                    { key: 'gre_cities_t10', label: 'greedy @t10', color: '#f87171' },
                    { key: 'gre_cities_t20', label: 'greedy @t20', color: '#b91c1c' },
                  ]}
                />
                ...
                {/* Tempo: map awareness, net vs Greedy */}
                <MetricChart
                  title="Tiles revealed @t20"
                  data={windowSize > 0 ? combinedRows.slice(-windowSize) : combinedRows}
                  series={[
                    { key: 'net_revealed_t20', label: 'net', color: '#22c55e' },
                    { key: 'gre_revealed_t20', label: 'greedy', color: '#f87171' },
                  ]}
              ...
              <div className="metrics-grid">
                {/* Tempo: income curve, net seat vs the Greedy anchor */}
                <MetricChart
                  title="SPT — net vs Greedy"
                  data={windowSize > 0 ? combinedRows.slice(-windowSize) : combinedRows}
                  series={[
                    { key: 'net_spt_t10', label: 'net @t10', color: '#22c55e' },
                    { key: 'net_spt_t20', label: 'net @t20', color: '#15803d' },
                    { key: 'gre_spt_t10', label: 'greedy @t10', color: '#f87171' },
                    { key: 'gre_spt_t20', label: 'greedy @t20', color: '#b91c1c' },
                  ]}
                />

                {/* SPT decomposed: cities x income-per-city. Distinguishes an
                expansion-pace deficit (low cities) from an income-efficiency
                deficit (low SPT/city) — conflating the two cost a lot of
                debugging time in the Jul 2026 tower investigation. */}
                <MetricChart
                  title="SPT per city — net vs Greedy"
                  data={windowSize > 0 ? combinedRows.slice(-windowSize) : combinedRows}
                  series={[
                    { key: 'net_spt_per_city_t10', label: 'net @t10', color: '#22c55e' },
                    { key: 'net_spt_per_city_t20', label: 'net @t20', color: '#15803d' },
                    { key: 'gre_spt_per_city_t10', label: 'greedy @t10', color: '#f87171' },
                    { key: 'gre_spt_per_city_t20', label: 'greedy @t20', color: '#b91c1c' },
                  ]}
                />

                {/* Economy Trajectory (net seats only since Jul 2026; older
                rows blended both players — greedy included in mixed games) */}
                <MetricChart
